...
for line in file:
    ...
    if 'кидать' == firstParsedWord.normal_form and ({'NOUN', 'accs'} in secondParsedWord.tag or
                     {'NPRO', 'accs'} in secondParsedWord.tag):  # проверка на винительный падеж(прямой объект)
        # print(ngramFields)
        ...
        try:
            ...
            dictionaryOfObjects[ngramFields[0][1]] = int(ngramFields[2])
    sys.stdout.write('\r')
    sys.stdout.write("Parsing progress: " + str(counter) + "  lines parsed |" + "Analysis progress: " + str(counter1) + " вхождения кидать " )
    sys.stdout.flush()
